...
dataset_dir = '/dataset/'
dataset_name = 'kitti_odom_gray/'  # 'kitti_odom_optflo/'
img_seq = '06/'  # 'training/'
cam1 = 'image_0/'  # 'image_2/'
cam2 = 'image_1/'  # 'image_2/'

for strOption, strArgument in \
        getopt.getopt(sys.argv[1:], '', [strParameter[2:] + '=' for strParameter in sys.argv[1::2]])[0]:
    if strOption == '--model' and strArgument != '': arguments_strModel = strArgument  # which model to use
    if strOption == '--one' and strArgument != '': cam1 = strArgument  # path to the first frame
    if strOption == '--two' and strArgument != '': cam2 = strArgument  # path to the second frame
    if strOption == '--seq' and strArgument != '': img_seq = strArgument  # path to where the output should be stored
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        # self.load_state_dict(torch.load('/home/jingkun/SemesterProject/pytorch-liteflownet/model/network-default.pytorch'))
...
##########################################################
def tensor_flo_to_file(ten_output, out_file):
    objOutput = open(out_file, 'wb')

    numpy.array([80, 73, 69, 72], numpy.uint8).tofile(objOutput)
    numpy.array([ten_output.shape[2], ten_output.shape[1]], numpy.int32).tofile(objOutput)
    numpy.array(ten_output.numpy().transpose(1, 2, 0), numpy.float32).tofile(objOutput)
...
def forward_backward(img_dir, out_dir):
    path = os.path.dirname(os.path.abspath(__file__)) + img_dir + '*.png'
    img_paths = sorted(glob.glob(path))

    # estimation on sequence
    timestamp = len(img_paths)  # rename this variable
    ...
    timer = 0.0
    print("Start estimating forward and backward optical flow")

    for i in range(timestamp - 1):
        start_time = time.time()

        tenOne, size_one = get_image_euroc(img_paths[i])
        tenTwo, size_two = get_image_euroc(img_paths[i + 1])
        # tenOne = numpy.array(PIL.Image.open(img_paths[i]))[:, :, ::-1]
        # tenTwo = numpy.array(PIL.Image.open(img_paths[i + 1]))[:, :, ::-1]
        ...
        # forward estimation
        tenOutput_forward = estimate(tenOne, tenTwo)
        # backward estimation
        tenOutput_backward = estimate(tenTwo, tenOne)

        timer += time.time() - start_time

        out_path_forward = os.path.dirname(os.path.abspath(__file__)) + out_dir + "forward/"
        make_dir_if_not_exist(out_path_forward)
        out_file_forward = os.path.join(out_path_forward, "{}.{}".format(os.path.basename(img_paths[i])[:-4], 'flo'))

        tensor_flo_to_file(tenOutput_forward, out_file_forward)

        out_path_backward = os.path.dirname(os.path.abspath(__file__)) + out_dir + "backward/"
        make_dir_if_not_exist(out_path_backward)
        out_file_backward = os.path.join(out_path_backward, "{}.{}".format(os.path.basename(img_paths[i])[:-4], 'flo'))

        tensor_flo_to_file(tenOutput_backward, out_file_backward)
...
def stereo(img_left_dir, img_right_dir, out_dir):
    # get the image pathname
    path_left = os.path.dirname(os.path.abspath(__file__)) + img_left_dir + '*.png'
    img_left_paths = sorted(glob.glob(path_left))

    path_right = os.path.dirname(os.path.abspath(__file__)) + img_right_dir + '*.png'
    img_right_paths = sorted(glob.glob(path_right))

    # estimation on sequence
    timestamp = len(img_left_paths)  # rename this variable
    ...
    timer = 0.0
    print("Start estimating stereo optical flow.")

    for i in range(timestamp):
        tenOne, size_one = get_image_euroc(img_left_paths[i])
        tenTwo, size_two = get_image_euroc(img_right_paths[i])
        # tenOne = numpy.array(PIL.Image.open(img_left_paths[i]))[:, :, ::-1]
        # tenTwo = numpy.array(PIL.Image.open(img_right_paths[i]))[:, :, ::-1]
        ...
        tenOutput_l2r = estimate(tenOne, tenTwo)

        out_path_flo_l2r = os.path.dirname(os.path.abspath(__file__)) + out_dir + "LToR/"
        make_dir_if_not_exist(out_path_flo_l2r)

        # out_file = os.path.join(out_path_flo, "{:06d}.{}".format(i, 'flo'))
        out_file_l2r = os.path.join(out_path_flo_l2r, "{}.{}".format(os.path.basename(img_left_paths[i])[:-4], 'flo'))

        tensor_flo_to_file(tenOutput_l2r, out_file_l2r)

        # right to left estimation
        tenOutput_r2l = estimate(tenTwo, tenOne)

        out_path_flo_r2l = os.path.dirname(os.path.abspath(__file__)) + out_dir + "RToL/"
        make_dir_if_not_exist(out_path_flo_r2l)

        # out_file = os.path.join(out_path_flo, "{:06d}.{}".format(i, 'flo'))
        out_file_r2l = os.path.join(out_path_flo_r2l, "{}.{}".format(os.path.basename(img_left_paths[i])[:-4], 'flo'))

        tensor_flo_to_file(tenOutput_r2l, out_file_r2l)
...
def main():
    ...
    arguments_strImgDir = dataset_dir + dataset_name + img_seq
    arguments_strImgOut = dataset_dir + dataset_name + img_seq
    # end

    # Inference forward backward optical flow
    forward_backward(arguments_strImgDir + cam1, arguments_strImgOut + 'image0_flo/')
    forward_backward(arguments_strImgDir + cam2, arguments_strImgOut + 'image1_flo/')

    # Inference stereo optical flow
    # stereo(arguments_strImgDir + cam1, arguments_strImgDir + cam2, arguments_strImgOut)

    print("Deep Optical Flow Estimation Done!")
    return True
